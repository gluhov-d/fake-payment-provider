...
In the scope of this task, we need to implement a **FAKE PAYMENT PROVIDER**. This service provides the ability to create real transactions (top up, withdrawal), send webhooks, and check the merchant balance.
...
## Transaction (Top Up)
The transaction method allows creating a top-up (deposit) transaction and charges the customer payment card.
### Summary
...
This method allows the creation of a top-up (deposit) transaction.
...
|---------------|--------|--------|-----------------------------------------------------------------------------|
| Authorization | string | header | Required - Unique parameter, which must be passed to all requests for identification. |

You must include the authorization parameter in your header when posting your request. The authorization header must contain the word `Basic` followed by a space and your credentials encoded in Base64 format. To encode your credentials, type your shop ID followed by a colon and your secret key, like so: merchantId:secretKey\
